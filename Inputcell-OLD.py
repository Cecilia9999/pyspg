...
def GetStandardCell(latt, pos, num, trans_matrix, center, is_prim, pgt_type, origin, prim_flag=False):
    ...
    if pgt_type[4] == 'CUBIC' or pgt_type[4] == 'ORTHO':
        ...
        if abs(nlb - nlc) < 0.001:
            std_latt[0] = [nlc / 2, -nlc * np.sqrt(3) / 2, 0]
            std_latt[1] = [nlc / 2, nlc * np.sqrt(3) / 2, 0]
            std_latt[2] = [0, 0, nla]
        else:
            std_latt[0] = [nla / 2, -nla * np.sqrt(3) / 2, 0]
            std_latt[1] = [nla / 2, nla * np.sqrt(3) / 2, 0]
            std_latt[2] = [0, 0, nlc]
        transf = np.eye(3, 3)
...
#t_m = np.dot(np.transpose(np.dot(prim_latt, np.linalg.inv(o_latt))), trans_matrix)   # P vertical:  p1 = o_latt --> delaunay prim; p2 = delaunay prim --> bravais latt; P = p1 * p2
#tp_positions = delaunay.ChangeOfBasis(o_pos, np.linalg.inv(t_m))  # // o_pos(prim) --> bravais(conv)
#tp1_positions = []
